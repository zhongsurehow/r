...
async def get_real_currency_comparison_data(symbols):
    """从CoinGecko获取真实货币比较数据"""
    try:
        # 获取货币列表数据
        ...
        for coin in coins_data:
            symbol_to_id[coin['symbol'].upper()] = coin
        ...
        for symbol in symbols:
            ...
            if symbol_upper in symbol_to_id:
                ...
                result.append({
                    'symbol': symbol_upper,
                    'name': coin.get('name', f'{symbol_upper} Token'),
                    'price': price,
                    '涨跌24h': change_24h,
                    'volume_24h': coin.get('total_volume', 0),
                    '市值': coin.get('market_cap', 0),
                    'market_cap_rank': coin.get('market_cap_rank', 999),
                    'circulating_supply': coin.get('circulating_supply', 0),
                    'total_supply': coin.get('total_supply', 0),
                    'max_supply': coin.get('max_supply'),
                    'ath': coin.get('ath', price),
                    'ath_change': coin.get('ath_change_percentage', 0),
                    'atl': coin.get('atl', price),
                    'atl_change': coin.get('atl_change_percentage', 0),
                    'rsi': rsi,
                    'volatility': volatility,
                    'sharpe_ratio': random.uniform(-1, 3),  # 需要更复杂的计算
                    'beta': random.uniform(0.5, 2.0)  # 需要更复杂的计算
                })
            else:
                # 如果找不到真实数据，使用虚拟数据
                result.append(generate_currency_data(symbol))
                
        return result
        
    except Exception as e:
        print(f"获取真实货币比较数据失败: {e}")
        return None
...
def main():
    ...
    # 获取货币数据（优先使用真实数据）
    cache_key = f"comparison_data_{','.join(sorted(new_currencies))}"
    if cache_key not in st.session_state or st.button("🔄 刷新比较数据", key="refresh_comparison_data"):
        with st.spinner("正在获取最新比较数据..."):
            try:
                # 尝试获取真实数据
                real_data = asyncio.run(get_real_currency_comparison_data(new_currencies))
                if real_data and len(real_data) > 0:
                    st.session_state[cache_key] = real_data
                    st.session_state[f"{cache_key}_source"] = 'real'
                    st.success(f"✅ 已获取 {len(real_data)} 个货币的真实比较数据")
                else:
                    # 如果真实数据获取失败，使用虚拟数据
                    st.session_state[cache_key] = [generate_currency_data(symbol) for symbol in new_currencies]
                    st.session_state[f"{cache_key}_source"] = 'mock'
    ...
    currencies_data = st.session_state[cache_key]
    
    # 显示数据源信息
    if st.session_state.get(f"{cache_key}_source") == 'real':
        st.info("📊 当前显示真实比较数据")
    else:
        st.warning("📊 当前显示模拟数据")
