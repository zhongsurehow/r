...
# 新增优化模块导入
from config.app_config import get_config, display_config_panel
from utils.performance_monitor import global_monitor, display_performance_dashboard, performance_monitor
from utils.exception_handler import exception_handler, display_error_dashboard, global_exception_handler
from utils.optimized_cache import async_cached, BatchProcessor, MemoryOptimizer
from utils.ui_enhancements import apply_custom_css, create_enhanced_dashboard, ModernUI, NotificationSystem
from utils.code_quality import check_code_quality, get_quality_metrics
from utils.unified_error_handler import global_error_handler, error_handler, safe_execute as unified_safe_execute
...
st.set_page_config(
    ...
    page_icon=app_config.ui.page_icon,
    layout=app_config.ui.layout,
    initial_sidebar_state=app_config.ui.sidebar_state
)
...
@exception_handler(show_user_message=False, default_return={})
@performance_monitor(global_monitor)
def get_config_data():
    """Load configuration from file and cache it."""
    config = get_config()
    return {
        'exchanges': config.trading.default_exchanges,
        'symbols': config.trading.default_symbols,
        'timeframes': ['1m', '5m', '15m', '1h', '4h', '1d'],
        'indicators': ['RSI', 'MACD', 'Bollinger Bands', 'Moving Average']
    }

@st.cache_resource(ttl=600)  # 10分钟缓存
def get_db_manager(db_path: str):
    """Creates and caches the database manager with optimized error handling."""
    if not db_path: 
        return None
...
def get_providers(_config: Dict, _session_state) -> List[BaseProvider]:
    """Create and cache a list of all data providers with optimized initialization."""
    providers = []
    ...
    for ex_id in _session_state.selected_exchanges:
        try:
            provider = CEXProvider(name=ex_id, config=provider_config, force_mock=is_demo_mode)
            providers.append(provider)
            logger.debug(f"成功初始化交易所提供者: {ex_id}")
        except ValueError as e:
            failed_exchanges.append((ex_id, "配置错误", str(e)))
            logger.error(f"CEX provider configuration error for {ex_id}: {e}")
        except ImportError as e:
            failed_exchanges.append((ex_id, "模块缺失", str(e)))
            logger.warning(f"CEX provider import error for {ex_id}: {e}")
        except ConnectionError as e:
            failed_exchanges.append((ex_id, "连接失败", "网络连接问题"))
            logger.warning(f"CEX provider connection error for {ex_id}: {e}")
        except Exception as e:
            failed_exchanges.append((ex_id, "未知错误", str(e)))
            logger.error(f"CEX provider unknown error for {ex_id}: {e}", exc_info=True)
    
    # 批量显示错误信息
    if failed_exchanges:
        with st.expander(f"⚠️ {len(failed_exchanges)} 个交易所初始化失败", expanded=False):
            for ex_id, error_type, error_msg in failed_exchanges:
                st.warning(f"**{ex_id}**: {error_type} - {error_msg}")
    
    logger.info(f"成功初始化 {len(providers)}/{len(_session_state.selected_exchanges)} 个交易所提供者")
    return providers
...
def show_dashboard(engine: ArbitrageEngine, providers: List[BaseProvider]):
    ...
    # 显示快速入门指南
    from components.quick_start_guide import show_quick_start_guide
...
def main():
    ...
    if st.session_state.get('show_trading', False):
        ...
        page = st.sidebar.selectbox(
            "📊 选择功能",
            ["🏠 实时仪表盘", "💼 专业交易界面", "🌍 货币比对中心", "📈 数据分析中心", "🔍 新货币监控", "⚙️ 系统设置", "📚 使用帮助", "🔍 性能监控", "🚨 错误统计", "📝 代码质量"],
            index=0
        )
        ...
        if page == "🏠 实时仪表盘":
            ...
            display_code_quality_dashboard()


@exception_handler
@performance_monitor
def display_code_quality_dashboard():
    """显示代码质量检查仪表板"""
    st.header("📝 代码质量检查")
    ...
    with col1:
        ...
        if st.button("🚀 开始质量检查", type="primary"):
            with st.spinner("正在分析代码质量..."):
                try:
                    # 获取项目根目录
                    project_path = "."
                    
                    # 生成质量报告
                    report = check_code_quality(project_path)
                    
                    # 显示报告
                    st.markdown(report)
                    
                    # 获取详细指标
                    metrics = get_quality_metrics(project_path)
                    ...
                    with col_b:
                        doc_coverage = (metrics.documented_functions / metrics.total_functions * 100) if metrics.total_functions > 0 else 0
                        ModernUI.create_metric_card(
                            title="文档覆盖率",
                            value=f"{doc_coverage:.1f}%",
                            delta=f"{doc_coverage - 70:.1f}%" if doc_coverage >= 70 else f"{doc_coverage - 70:.1f}%",
                            delta_color="normal" if doc_coverage >= 70 else "inverse",
                            icon="📚",
                            help_text="函数文档字符串覆盖率"
                        )
                    
                    with col_c:
                        type_coverage = (metrics.type_annotated_functions / metrics.total_functions * 100) if metrics.total_functions > 0 else 0
                        ModernUI.create_metric_card(
                            title="类型注解覆盖率",
                            value=f"{type_coverage:.1f}%",
                            delta=f"{type_coverage - 80:.1f}%" if type_coverage >= 80 else f"{type_coverage - 80:.1f}%",
                            delta_color="normal" if type_coverage >= 80 else "inverse",
                            icon="🏷️",
                            help_text="函数类型注解覆盖率"
                        )
                    ...
                    with col_d:
                        ModernUI.create_metric_card(
                            title="平均复杂度",
                            value=f"{metrics.complexity_score:.1f}",
                            delta=f"{5.0 - metrics.complexity_score:.1f}" if metrics.complexity_score <= 5.0 else f"{5.0 - metrics.complexity_score:.1f}",
                            delta_color="normal" if metrics.complexity_score <= 5.0 else "inverse",
                            icon="🔄",
                            help_text="平均圈复杂度"
                        )
                    
                    # 问题分布图表
                    if metrics.issues_count:
                        st.subheader("🐛 问题分布")
                        ...
                        issues_df = pd.DataFrame(
                            list(metrics.issues_count.items()),
                            columns=['问题类型', '数量']
                        )
    ...
    with col2:
        ...
        **D (<60分)**
        ...
        if st.button("📊 生成详细报告", key="detailed_report"):
            st.info("详细报告功能开发中...")
        
        if st.button("🔧 自动修复", key="auto_fix"):
            st.info("自动修复功能开发中...")
